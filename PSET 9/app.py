...
def index():
    ...
    return render_template("index.html", rows=rows, name=name, prices=prices, total=total, sum=sum, cash=cash)
...
def buy():
    ...
    if request.method == "POST":
        ...
        if array1 == []:
            db.execute("INSERT INTO purchases(user_id, price, shares, time, type, symbol) VALUES(?,?,?,?,?,?)", id, price, shares, time, transaction_type, input.upper())

        for s in array1:
            if symbol_sign == s["symbol"]:
                old_shares = db.execute("SELECT shares FROM purchases WHERE user_id = ? AND symbol == ?", id, symbol_sign)
                number = int(number) + int(old_shares[0]["shares"])
                db.execute("UPDATE purchases SET shares = ? WHERE user_id = ?", number, id)
...
def register():
    ...
    if request.method == "POST":
        ...
        if password_register != password_confirmation or len(password_register) == 0 or len(username) == 0:
        ...
        db.execute("INSERT INTO users(username, hash, cash) VALUES(?, ?,?)", username, hash, amount)
        flash('Sucessfully')
        return render_template("login.html")
...
def sell():
    ...
    if request.method == "POST":
        ...
        if len(input) == 0:
            return apology("Blank input", 403)
                ...
                if int(number_sold) <= int(s["shares"]):
                    ...
                    for row in rows:
                        if input == row["symbol"]:
                            ...
                            db.execute("UPDATE purchases SET shares = ? WHERE user_id = ?", new_shares, id)

                    updated = db.execute("SELECT * FROM purchases WHERE user_id LIKE ?", id)
                    ...
                    new_balance = db.execute("SELECT cash FROM users WHERE id = ?", id)
                    cash = float(new_balance[0]["cash"])
                    return render_template("index.html", rows=updated, name=name, prices=prices, total=total, sum=sum, cash=cash)

            return apology("stock or invalid number of shares", 403)
